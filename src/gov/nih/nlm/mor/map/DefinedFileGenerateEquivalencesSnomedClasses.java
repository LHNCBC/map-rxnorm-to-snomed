...
public class DefinedFileGenerateEquivalencesSnomedClasses implements java.io.Serializable {

	private static final long serialVersionUID = 3324801368865331845L;
	Constants Consts = new Constants();
	OWLOntologyManager man = null;
	OWLOntology ontology = null;
	OWLOntology ontologyForJim = null;
	...
	final String snomedNamespace = new String("http://snomed.info/id/");
...
//	public Vector<String> seen = new Vector<String>();
	...
	private RestUrl restUrl = new RestUrl();
	
	
	@SuppressWarnings("deprecation")
	public DefinedFileGenerateEquivalencesSnomedClasses(String filename, FetchRxNormDataAllStatus fetchRxNorm, boolean addGcis, boolean debug) throws OWLOntologyStorageException {
		...
		try {
			...
			ontologyForJim = man.createOntology(IRI.create(rxNamespace));
			
			SetOntologyID change = new SetOntologyID(ontologyForJim, versionIRI);
			
			ontologyForJim.getOWLOntologyManager().applyChange(change);			
...
//		}
		...
		scd2SBDs = fetchRxNorm.getScd2SBDs();
		cui2SBDs = fetchRxNorm.getCui2SBD();
		
		//don't forget to prefix (for now - will want to change default namespace to http://mor../\d+)
		Set<String> allSbdCuis = cui2SBDs.keySet();
...
//		OWLClass snomedNumber = factory.getOWLClass(namespace, "260299005");
		...
		OWLClass fakeSBDParent = factory.getOWLClass(snomedNamespace, df.format(++snomedCodeGenerator) + "-FS");
		OWLClass fakeIngredientParent = factory.getOWLClass(snomedNamespace, df.format(++snomedCodeGenerator) + "-FS");
		OWLClass fakeSBDParentNoScd = factory.getOWLClass(snomedNamespace, df.format(++snomedCodeGenerator) + "-FS");
...
//		addFakeParent(fakeNumberParent, snomedNumber, "Untranslated Numbers");
		...
		addFakeParent(fakeSBDParentNoScd, snomedMpParent, "RXNORM WIP SBDs (Brand Drugs)");
...
//			}
		...
		for(OWLAxiom axiom : unitsOfMeasureAxioms) {
			...
			man.addAxiom(ontologyForJim, axiom);
		}
...
//TODO: Only if asked - pharm-archaeology based on inference from Active concepts with similar features		
//		//report non-Active missing definitional features with inferred AI from a similar matching SCD
//		// there are over 11,000 detected without. will require matching the RXN STR without strengths
//		//or a similar scoring method.
//
//		for(String scdCui : fetchRxNorm.getCui2SCD().keySet()) {
//			RxNormSCD scd = fetchRxNorm.getCui2SCD().get(scdCui);
//			if(!scd.getStatus().equalsIgnoreCase("active")) {
//				Vector<RxNormBoss> bosses = scd.getRxNormBoss();
//				int totalBoss = bosses.size();
//				for(RxNormBoss boss : bosses) {
//					if(boss.getActiveIngredientRxCui() == -1) {
//						RxNormSCD tempSCD = findSimilarActiveSCD(scd, fetchRxNorm.getCui2SCD());
//						if(tempSCD.getRxNormBoss().size() == totalBoss) {
//							//ok, the size is right - and it was derived from an Active SCD
//						}
//						else {
//							//a similar scd doesn't exist with the name and strengths
//							break;
//						}
//					}
//				}
...
//		}
		...
		Set<OWLAxiom> rxClinicalDrugAxioms = generateClinicalDrugAxioms(fetchRxNorm.getCui2SCD(), fakeSBDParent, fakeSBDParentNoScd, fakeCdParent, fetchRxNorm.getCui2SBD(), allSbdCuis);
		man.addAxioms(ontology, rxClinicalDrugAxioms.stream());
		man.addAxioms(ontologyForJim, rxClinicalDrugAxioms.stream());
...
//221223: I don't want others removing content based on a keyword.
//If this is to be done we need to know what kind of flag should exist on the concept - currently, I think the existence of the SNOMED hasDbXref (or something similar) would suffice.
	...
	private void addMissingSubstanceMappings() {
		for( String key : jnMissingSubstanceMappings.keySet() ) {
			...
			for(String substance : substances ) {
				...
				man.addAxiom(ontologyForJim, eq);
			}
		}		
	}
...
//		return c.getIRI().getIRIString().replace(namespace, "");
	...
	private void createAnnotation(OWLAnnotationProperty ap, String s) {
		...
		man.addAxiom(ontologyForJim, labelToClass);
	}
	...
	private void addAnnotation(OWLClass c, OWLAnnotationProperty ap, boolean filler) {
		...
		man.addAxiom(ontologyForJim, annoToClass);
	}	
	...
	private void addMapsAnnotation(OWLClass snomed, OWLClass rx) {
		...
		if( code != null && name != null) {
			...
			OWLAnnotationProperty annop = factory.getOWLAnnotationProperty(rxNamespace, "MapsToCode");
			...
			annop = factory.getOWLAnnotationProperty(rxNamespace, "MapsToName");
...
//			System.out.println("Add maps to annos");
			...
			man.addAxioms(ontologyForJim, axioms.stream());
		}
	...
	public void addFakeParent(OWLClass fakeParent, OWLClass snomedParent, String labelForFake) {
		...
		man.addAxiom(ontologyForJim, labelToClass);
		man.addAxiom(ontologyForJim, subClassAxiom);		
		reasoner.flush();
	}
...
//		log("STEP E");
	...
	private void retree(OWLClass a, OWLClass destination) {
		...
		man.addAxiom(ontologyForJim, newSubClass);
	}
	...
	public void setConstants() {
		...
		this.isModificationOf = factory.getOWLObjectProperty(snomedNamespace, "738774007");
...
//			annotationCodeClasses.put(c, ap);
//			createAnnotation(ap, c);			
//		}
...
//		addCodeAnnotationAssertions();
		...
		man.addAxiom(ontologyForJim, subStartNdc);		
		man.addAxiom(ontology, subEndNdc);	
		man.addAxiom(ontologyForJim, subEndNdc);	
	}
...
//		numberClassToName.put(val, cls);
	...
	public Set<OWLAxiom> generateNumberAxioms(Set<String> numberSet, Set<OWLClass> snomedNumberClasses, OWLClass parent) {
	    ...
	    //numerics such as xsd:decimal. This was a decision to support SOLOR (SNOMED O. LOINC O. RxNorm)
		...
		for( String numberString : numberSet ) {
			...
			if( !snomedNumberQualifier.containsKey(labelString) && NumberUtils.isNumber(labelString) ) {
				OWLClass snClass = factory.getOWLClass(snomedNamespace, df.format(++snomedCodeGenerator) + "-FS");
				OWLAnnotation label = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(labelString));
	...
	public Set<OWLAxiom> generateClinicalDrugAxioms(TreeMap<String, RxNormSCD> scdMap, OWLClass fakeSBDParent, OWLClass fakeSBDParentNoSbd, OWLClass fakeCdParent, TreeMap<String, RxNormSBD> sbdCuis, Set<String> allSbdCuis) {

		Set<OWLAxiom> axioms = new HashSet<>();
		int scdCount = 0;
		//For SBD QA
		Set<String> sbdCuiSet = sbdCuis.keySet();
...
// There are two contructors in each the drug product classes
// For the defined file we don't care about this annotation as by default it's true, however
// properties are removed from non-Active products. 
...
//			}
			...
			//special cases for the injectables
//			if( doseFormCui.equals("1649574") ) {
//				if( !scd.getRxNormQuantityFactor().isEmpty() ) {
//					//a Quantity Factor exists for the injection
//					//so we know there should be both concentration 
//					//and presentation predicates in the BoSS(s)
////					scd.getSnomedDefinitionTemplate().setIsPresentation(true);
////					scd.getSnomedDefinitionTemplate().setIsConcentration(true);
//					scd.setSnomedDefinitionTemplate(new SnomedDefinitionTemplate("pc"));
//				}
//				else {
////					scd.getSnomedDefinitionTemplate().setIsPresentation(true);
////					scd.getSnomedDefinitionTemplate().setIsConcentration(false);
//					scd.setSnomedDefinitionTemplate(new SnomedDefinitionTemplate("p"));					
//				}				
//			}
//			else if( doseFormCui.equals("316950") || doseFormCui.equals("316949") ) {
//				if( scd.getRxNormQuantityFactor().isEmpty() ) {
////					scd.getSnomedDefinitionTemplate().setIsConcentration(true);
////					scd.getSnomedDefinitionTemplate().setIsPresentation(false);
//					scd.setSnomedDefinitionTemplate(new SnomedDefinitionTemplate("c"));					
//				}
//				else {
//					//what???????????
//					log("This Injectable solution/suspension has a quantity factor");
//					log(false);
//					applyDefinition = false;
//				}
//			}
...
//					}
					...
					if( boss.getActiveIngredientRxCui() != -1 ) {
						...
						//We don't want to mess up Jim's work because content doesn't exist for translation in RxNorm
						//so do not create SCDs where AIs don't exist in RxNorm
						//TODO: OK, try this bc Jim wants AIs on obsoletes where they don't exist. See if siblings yield clues.
						//RESULT: Didn't work because MPFs express the AI (not the Base), thus no slot currently exists for an inference
						applyDefinition = false;
...
//						OWLClass integerClass = getOWLClassForNumberString(String.valueOf(size), snomedNumberQualifier);
				...
				if( applyDefinition ) {
					...
					String scdCui = scd.getCui().toString();
					
					ArrayList<RxNormSBD> sbds = scd2SBDs.get(scdCui);
					Set<RxNormSBD> allSbds = new HashSet<>();
					
					ArrayList<OWLClass> sbdClasses = new ArrayList<OWLClass>();					
				
					if(sbds != null) {	
						for(RxNormSBD knownSbd : sbds ) {
							allSbds.add(knownSbd);
						}
						for(RxNormSBD b : sbds) {
							for(RxNormSBD q : b.getQuantifiedSbds()) {
								allSbds.add(q);							
							}
							
							for(RxNormSBD r : b.getRemappedSbds()) {
								allSbds.add(r);
							}
						}
										
						for(RxNormSBD sbd : allSbds) {
							sbdCuiSet.remove(sbd.getRxcui().toString());
							OWLClass sbdClass = createSBDClass(sbd);
							applyProperties(sbdClass, sbd.getProperties());
							applyNdcs(sbdClass, sbd.getNdcSet());
							OWLSubClassOfAxiom sbdSubClassAxiom = factory.getOWLSubClassOfAxiom(sbdClass, scdClass);							
							axioms.add(sbdSubClassAxiom);
							sbdClasses.add(sbdClass);
						}
					} 
				
					allSBDs.addAll(sbdClasses);
					...
					if(sbds != null) {	
						...
						for(RxNormSBD sbd : allSbds) {
							...
							applyNdcs(sbdClass, sbd.getNdcSet());
							OWLSubClassOfAxiom sbdSubClassAxiom = factory.getOWLSubClassOfAxiom(sbdClass, scdClass);
							OWLSubClassOfAxiom sbdFakeParentAxiom = factory.getOWLSubClassOfAxiom(sbdClass, fakeSBDParent);
							
							axioms.add(sbdSubClassAxiom);
							axioms.add(sbdFakeParentAxiom);
							sbdClasses.add(sbdClass);
						}
					}			
				...
				//add the missing ~7k SBDs here with DFs not in SNCT to (a presumably existing SCD Class since all are counted for)
				//if this works modularize into a method for the two blocks above
				String scdCui = scd.getCui().toString();
...
//				if(sbdsMapped != null) {						
//					for(RxNormSBD sbd : sbdsMapped) {
//						for(RxNormSBD b : sbdsMapped) {
//							for(RxNormSBD q : b.getQuantifiedSbds()) {
//								quantified.add(q);							
//							}
//							
...
//						}
//						
//						sbds.addAll(quantified);
//						sbds.addAll(remapped);						
//						sbdCuiSet.remove(sbd.getRxcui().toString());							
//						OWLClass sbdClass = createSBDClass(sbd);
//						applyProperties(sbdClass, sbd.getProperties());
//						applyNdcs(sbdClass, sbd.getNdcSet());
//						OWLSubClassOfAxiom sbdSubClassAxiom = factory.getOWLSubClassOfAxiom(sbdClass, scdClass);
//						OWLSubClassOfAxiom scdSubClassAxiom = factory.getOWLSubClassOfAxiom(scdClass, fakeCdParent);
//						OWLSubClassOfAxiom sbdFakeParentAxiom = factory.getOWLSubClassOfAxiom(sbdClass, fakeSBDParent);
//						
//						axioms.add(sbdSubClassAxiom);
//						axioms.add(scdSubClassAxiom);
//						axioms.add(sbdFakeParentAxiom);
//						sbdClasses.add(sbdClass);
//					}
//					allSBDs.addAll(sbdClasses);						
//				} 					
		...
		//QA which SBDs we are missing
		printMissingSBDs(sbdCuiSet, sbdCuis);
		//and repurpose the method, appropriately, to tree them in a WIP bucket
		axioms.addAll(treeMissingSBDs(sbdCuiSet, sbdCuis, fakeSBDParentNoSbd));
		
		return axioms;
		
	}
	
	private void printMissingSBDs(Set<String> sbds, TreeMap<String, RxNormSBD> sbdCuiMap) {
		PrintWriter sbdPw = null;
		try {
			sbdPw = new PrintWriter(new File("sbd-missing-qa.txt"));
		} catch(Exception e) {
			e.printStackTrace();
		}
		for(String cui : sbds) {
			RxNormSBD sbd = sbdCuiMap.get(cui);
			sbdPw.println(cui + "\t" + sbd.getStatus() + "\t" + sbd.getScdRxcuis().size() +  "\t" + sbd.getName());
		}
		sbdPw.flush();
		sbdPw.close();
	}
	
	private Set<OWLAxiom> treeMissingSBDs(Set<String> untreedSbdCuis, TreeMap<String, RxNormSBD> sbdCuiMap, OWLClass fakeSBDParentNoScd) {
		Set<OWLAxiom> sbdWipAxioms = new HashSet<>();
		
		for(String cui : untreedSbdCuis) {	
			RxNormSBD sbd = sbdCuiMap.get(cui);
			OWLClass sbdClass = createSBDClass(sbd);
			applyProperties(sbdClass, sbd.getProperties());
			applyNdcs(sbdClass, sbd.getNdcSet());
			OWLSubClassOfAxiom sbdFakeWipParentAxiom = factory.getOWLSubClassOfAxiom(sbdClass, fakeSBDParentNoScd);
			sbdWipAxioms.add(sbdFakeWipParentAxiom);		
		}
...
//			OWLAxiom annoToClass = factory.getOWLAnnotationAssertionAxiom(c.getIRI(), anno);			
		...
		if(!ndcsForClass.isEmpty()) {
			...
			man.addAxioms(ontologyForJim, ndcsForClass.stream());
		}
	...
	/*
	 * 	public void createSBDChildren(Integer code, RxNormSCD rxSCD, OWLClass fakeSBDParent) {
		Vector<RxNormSBD> brandDrugs = rxSCD.getBrandDrugs();
		
		OWLClass scdClass = factory.getOWLClass(namespace, "Rx" + code.toString());
		for(RxNormSBD sbd : brandDrugs) {
			...
			OWLSubClassOfAxiom sbdSubClassAxiom = factory.getOWLSubClassOfAxiom(sbdClass, scdClass);
			OWLSubClassOfAxiom sbdFakeSubClassAxiom = factory.getOWLSubClassOfAxiom(sbdClass, fakeSBDParent);
			
			man.addAxiom(ontology, classLabel);
			man.addAxiom(ontology, sbdSubClassAxiom);
			man.addAxiom(ontology, sbdFakeSubClassAxiom);
		}
	}
	 */
	public Set<OWLClass> createSBDClasses(RxNormSCD scd) {
		...
		for(RxNormSBD sbd : scd.getBrandDrugs()) {
			...
			OWLAnnotation labelForClass = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(sbd.getName() + " (RxNorm:" + sbd.getRxcui() + ")"));
			OWLAxiom classLabel = factory.getOWLAnnotationAssertionAxiom(sbdClass.getIRI(), labelForClass);
			
			man.addAxiom(ontology, classLabel);
			man.addAxiom(ontologyForJim, classLabel);
...
//	}
	...
	public void createDoseFormChild(Integer code, RxNormDoseForm df, OWLClass parent) {
		...
		OWLAnnotation labelForClass = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(df.getName() + " (RxNorm:" + df.getRxcui() + ")"));
		...
		man.addAxiom(ontologyForJim, classLabel);
		man.addAxiom(ontologyForJim, subClassOfAxiom);		
	}
	
	//TODO: For allProperties
...
//			}
			...
			//request by JC 23-03-26
			ingName = ingName.concat(" (RxNorm:" + ingredient.getRxcui() + ")");
			...
			man.addAxiom(ontologyForJim, classLabel);
			man.addAxiom(ontologyForJim, ingredientSubClassAxiom);			
		}
		
	}
	
//	public void createIngredientChild(Integer code, RxNormIngredient ingredient, OWLClass substanceParent, OWLClass fakeIngredientParent) {
//		if(code.toString().equals("10402") || code.toString().equals("105669")) {
...
//		if( ingName != null ) {
//			
//			OWLAnnotation labelForClass = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(ingredient.getName()));
//			OWLAxiom classLabel = factory.getOWLAnnotationAssertionAxiom(ingredientClass.getIRI(), labelForClass);
////			addAnnotation(ingredientClass, this.hasNDC, true));
//			
//			if( ingredient.getIsAllergenic() ) {
//				addAnnotation(ingredientClass, this.allergenic, true);
//				log("Adding allergenic annotation: " + ingredient.getName() + " : " + ingredient.getRxcui());
...
//
//			ArrayList<String> wrongEqIngs = new ArrayList<String>();
//			if( jnWrongSubstanceMappings.containsKey(rxCode) ) {
//				wrongEqIngs = jnWrongSubstanceMappings.get(rxCode);
//			}
//			
//			Vector<Long> snomedIngredients = ingredient.getSnomedCodes();			
//			
//			for( int i=0; i < snomedIngredients.size(); i++) {
//				OWLClass snomedClass = factory.getOWLClass(namespace, String.valueOf(snomedIngredients.get(i)));
//				String snomedLabel = getRDFSLabel(snomedClass);
//				String snomedCode = getCodeForClass(snomedClass);
//				//RWW 20200117: suppress incorrect mappings from the audit process				
//				if( !wrongEqIngs.isEmpty() && wrongEqIngs.contains(snomedCode) ) {
//					isWrong = true;
//				}
//				if( snomedLabel != null && snomedLabel.contains(" (substance)") && !isWrong ) {
//					OWLEquivalentClassesAxiom eq = factory.getOWLEquivalentClassesAxiom(ingredientClass, snomedClass);
//					man.addAxiom(ontology, eq);
//					man.addAxiom(ontologyForJim, eq);					
//					if( snomedClass != null && ingredientClass != null) addMapsAnnotation(ingredientClass, snomedClass);
...
//				String label = "";
//				if(isWrong) {
//					label = ingredient.getName() + " (Fake)";
//					exists = false;
//				}
//				else {
//					label = ingredient.getName();
//				}
//				labelForClass = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(label));
//				classLabel = factory.getOWLAnnotationAssertionAxiom(ingredientClass.getIRI(), labelForClass);
//				
//				if(!exists) {
//					addAnnotation(ingredientClass, this.substanceNoExist, "substance does not exist in SCT");
//					addAnnotation(ingredientClass, this.asserted, false);
//				}
//			}
//			
//			OWLSubClassOfAxiom ingredientSubClassAxiom = factory.getOWLSubClassOfAxiom(ingredientClass, substanceParent);				
//			man.addAxiom(ontology, classLabel);
//			man.addAxiom(ontology, ingredientSubClassAxiom);
//			man.addAxiom(ontologyForJim, classLabel);
//			man.addAxiom(ontologyForJim, ingredientSubClassAxiom);			
...
//	}
	...
	public void createSCDChild(String code, RxNormSCD scd, OWLClass scdParent) {
		...
		OWLAnnotation labelForClass = factory.getOWLAnnotation(factory.getRDFSLabel(), factory.getOWLLiteral(scd.getName() + " (RxNorm:" + scd.getCui() + ")"));
		...
		man.addAxiom(ontologyForJim, classLabel);
		man.addAxiom(ontologyForJim, scdSubClassAxiom);		
	}
...
//		//make a system call to the perl script, i don't have time for
//		//java regexs right now.
//		try {
//			nextFile = replaceRxCodesWithFS(fileString);
//		} catch (IOException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
	...
	public File saveJimOntology() {
		File file = null;
		String fileString = null;
		try {
			String globalDate = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			globalDate = globalDate.substring(0, 10);
			fileString = "./Defined-RxNorm-with-SNCT-classes-" +  globalDate + ".owl";
			file = new File(fileString);
			
			FunctionalSyntaxDocumentFormat owlDocumentFormat = getFunctionalSyntaxDocumentFormat();
			ontologyForJim.getOWLOntologyManager().setOntologyFormat(ontologyForJim, owlDocumentFormat);
			
			man.saveOntology(ontologyForJim, IRI.create(file));
			System.out.println("Saving file as " + fileString);
		}	catch(OWLOntologyStorageException e) {
			e.printStackTrace();
		}
...
//		prefixManager.setDefaultPrefix(snomedNamespace);
//		prefixManager.setPrefix("oboInOwl", oboInOwlNamespace);		
...
//		return c.getIRI().getIRIString().contains("Rx");
	...
	private String replaceRxCodesWithFS(String filename) throws IOException {
		Runtime r = Runtime.getRuntime();
		Process p = r.exec("perl refactorIdentifiers.pl " + filename + " " + filename.replace("_With_Fake_Snomed", "_Without_Rx_Ids"));
		try {
			p.waitFor();
		...
		return filename.replace("_With_Fake_Snomed", "_Without_Rx_Ids");
	}
...
//	}
	...
	public JSONObject getresult(String URLtoRead, ExponentialBackoff b) throws IOException, InterruptedException {
		URL url;
		HttpURLConnection connexion;
		BufferedReader reader;
...
//		OWLClass cls = factory.getOWLClass(namespace, String.valueOf(code));
	...
	private boolean isAsserted(OWLClass c) {
	...
	private boolean isInferred(OWLClass c) {
		boolean isInferred = false;
		if( inferredClasses.contains(c)) {
			isInferred = true;
		}
		return isInferred;
	}
...
// https://github.com/IHTSDO/snomed-owl-toolkit/blob/988cdc54fddb750424ffe21b4a2a1029412eb0bc/src/main/java/org/snomed/otf/owltoolkit/ontology/OntologyService.java
	...
	private void applyProperties(OWLClass c, Set<Property> properties) {
		for(Property p : properties) {
			...
			case("ATTRIBUTES"):
				...
				break;
			case("CODES"):
				//and for CODES I'd like to use hasDbXref for meaningful use 
				//and also to remove ambiguity amongst attribute names overlapping with idTypes (MORP-2304)
...
//	private void applyNdcs(OWLClass c, Set<NDC> ndcs) {
//		for(NDC n : ndcs) {
//			addNdcAnnotations(c, n);
//			//TODO: create a method to add status, startDate, and endData as annotationSubProperties				
//		}		
//	}
//	
//	private void addNdcAnnotation(OWLClass c, NDC ndcObject) {
//		
//	}
...
//TODO: Only if asked. This is going to be a larger algorithm than I thought. The obsolete/non-Active
//      SCD can't be guaranteed to have any definitional features in common with the most similar (even if in the DFG)
//  Example: the obsolete SCD ketorolac 20 MG Oral Tablet (248650)l
//	          has a different base than the active SCD (IN/PIN difference) 834022	ketorolac tromethamine 10 MG Oral Tablet 
//	private RxNormSCD findSimilarActiveSCD(RxNormSCD scd, TreeMap<String, RxNormSCD> scdMap) {
//		for(String cui : scdMap) {
//			RxNormSCD tempScd = scdMap.get(cui);
//			if(tempScd.getStatus().equalsIgnoreCase("active")) {
//				if(tempScd.getRxNormDoseForm().get(0).equals(scd.getRxNormDoseForm().get(0))) {
//					
//				}
//			}
//		}
